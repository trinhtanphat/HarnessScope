...
fn har_import_matches_node_semantics_and_redacts_before_return() {
    ...
    assert_eq!(events[0].timestamp_utc.as_deref(), Some("2026-09-04T00:00:00.000Z"));
    assert_eq!(events[1].timestamp_utc, events[0].timestamp_utc);
    assert_eq!(events[0].correlation_id, events[1].correlation_id);
    assert!(events[0].correlation_id.as_deref().is_some_and(|value| !value.is_empty()));
    ...
    for secret in ["supersecret", "secret-query", "secret-cookie", "secret-body"] {
...
fn procmon_import_matches_node_operation_and_timestamp_mapping() {
    let events = import_procmon(&fixture("sample-procmon.csv"), "2026-09-04").expect("Procmon import");
    assert_eq!(events.iter().map(|event| event.kind.as_str()).collect::<Vec<_>>(), vec!["ProcessStarted", "FileRead", "FileWritten"]);
    assert_eq!(events[0].timestamp_utc.as_deref(), Some("2026-09-04T00:00:01.000Z"));
...
fn jsonl_import_matches_small_mapping_grammar_and_known_kind_names() {
    let events = import_jsonl(&fixture("sample.jsonl"), &fixture("sample-map.yaml")).expect("JSONL import");
    assert_eq!(events.iter().map(|event| event.kind.as_str()).collect::<Vec<_>>(), vec!["PermissionPrompt", "PermissionDecision"]);
    assert_eq!(events[0].source, "external-jsonl");
    assert_eq!(events[0].correlation_id.as_deref(), Some("p1"));
    assert_eq!(events[0].timestamp_utc.as_deref(), Some("2026-09-04T00:00:02.000Z"));
