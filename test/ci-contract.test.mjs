...
test('CI has portable Node and Rust matrices, native collector gates, parity, and Tauri package gates', () => {
  ...
  assert.match(workflow, /collector-linux:/);
  assert.match(workflow, /collector-macos:/);
...
test('native CI validates normalized v0.4 Tauri artifacts and uploads both platform bundles', () => {
  ...
  assert.match(workflow, /HarnessScope-0\.4\.0-windows-x64-Setup\.exe/);
  assert.match(workflow, /HarnessScope-0\.4\.0-windows-x64\.msi/);
  assert.match(workflow, /HarnessScope-0\.4\.0-windows-x64-portable\.zip/);
  assert.match(workflow, /HarnessScope-0\.4\.0-macos-universal\.dmg/);
  assert.match(workflow, /HarnessScope-0\.4\.0-macos-universal\.app\.zip/);
