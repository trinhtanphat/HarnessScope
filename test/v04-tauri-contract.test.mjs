...
const required = [
  'apps/tauri/README.md', 'apps/tauri/src-tauri/Cargo.toml', 'apps/tauri/src-tauri/build.rs',
  'apps/tauri/src-tauri/tauri.conf.json', 'apps/tauri/src-tauri/capabilities/main.json',
  'apps/tauri/src-tauri/src/main.rs', 'apps/tauri/src-tauri/src/commands.rs',
  'apps/tauri/src-tauri/src/state.rs', 'apps/tauri/src-tauri/src/errors.rs',
  ...icons.map((icon) => `apps/tauri/src-tauri/${icon}`), 'ui/tauri-bridge.js'
];
const commands = [
  ...
  'launch_run', 'export_run', 'collector_list', 'collector_describe', 'collector_start',
  'collector_stop', 'collector_status', 'dialog_pick_directory', 'dialog_pick_file'
];

test('v0.4 Tauri shell files, icons, collector commands, and least-privilege contract exist', () => {
  ...
  assert.match(cargo, /^version\s*=\s*"0\.4\.0"$/m);
...
test('v0.4 package scripts preserve Electron fallback and exact Tauri/collector entrypoints', () => {
